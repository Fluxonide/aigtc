...
import { PROVIDERS as META_PROVIDERS } from "@aigtc/meta";
import { PROVIDERS as REGISTRY_PROVIDERS } from "./registry.ts";

/**
 * Ensures @aigtc/meta's PROVIDERS (display metadata) and the runtime
...
describe("provider sync", () => {
  ...
  test("every runtime provider has a matching @aigtc/meta entry", () => {
    for (const id of registryIds) {
      expect(metaIds.has(id)).toBe(true);
    }
  });

  test("every @aigtc/meta provider has a matching runtime registry entry", () => {
