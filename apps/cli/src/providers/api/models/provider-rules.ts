...
export const PROVIDER_MODEL_RULES: Record<SupportedAPIProviderId, ProviderModelRules> = {
  ...
  kiro: {
    includePatterns: [/^(auto|claude-)/i],
    excludePatterns: [],
    dedupeKey: (modelId: string) => modelId,
    resolveCatalogProvider: () => null, // Not in models.dev catalog
    resolveTier: () => "default", // All Kiro models are top-tier
  },
};
