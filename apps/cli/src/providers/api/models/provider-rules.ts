  ...
  const catalogProvider =
    ctx.providerId === "openrouter"
      ...
      : ctx.providerId === "google-ai-studio"
        ? "google"
        : ctx.providerId === "cerebras" || ctx.providerId === "kiro"
          ? null
          : ctx.providerId;
...
export const PROVIDER_MODEL_RULES: Record<SupportedAPIProviderId, ProviderModelRules> = {
  ...
  kiro: {
    includePatterns: [/^claude-/i, /^auto$/i, /^deepseek-/i, /^qwen/i],
    excludePatterns: [],
    dedupeKey: (modelId: string) => {
      return modelId.replace(/-thinking$/i, "");
    },
    resolveCatalogProvider: () => null,
    resolveTier: (ctx: ProviderModelRuleContext) => {
      const id = ctx.model.id.toLowerCase();
      if (/auto/.test(id)) return "default";
      if (/haiku|mini/.test(id)) return "fast";
      if (/thinking|opus/.test(id)) return "reasoning";
      if (/sonnet/.test(id)) return "default";
      return "other";
    },
  },
};
